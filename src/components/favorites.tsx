...
function Favorites({offers}: FavoritesProps): JSX.Element {
	...
	const amsterdamOffers = favoriteOffers;
					...
					{amsterdamOffers.length > 0 && (
						...
						<article key={offer.id} className="favorites__card place-card">
							...
							<div className="place-card__price-wrapper">
								...
								<span className="place-card__price-text">&#47;&nbsp;{offer.dayOrNight}</span>
								</div>
								<button 
								className={`place-card__bookmark-button button ${offer.isBookmarks ? 'place-card__bookmark-button--active' : ''}`} 
								type="button"
								>
							...
							<p className="place-card__type">{offer.typeOfApartment}</p>
							</div>
						</article>
						))}
					</div>
